...
def calculate_time_saved_by_removing(route, index, departure_times, arrival_times):
    """
    Calculate the time saved by removing a place from the route.
    
    Args:
        route: List of places in the route.
        index: Index of the place to remove.
        departure_times: List of departure times for each place.
        arrival_times: List of arrival times for each place.
    
    Returns:
        Time saved in minutes by removing the place.
    """
    if index <= 0 or index >= len(route) - 1:
        return 0  # Cannot remove the start or end (hotel)
    
    # Calculate the travel time between the previous and next places
    previous_place = route[index - 1]
    next_place = route[index + 1]
    travel_time_without_place = abs(arrival_times[index + 1] - departure_times[index - 1])
    
    # Calculate the travel time with the current place
    travel_time_with_place = abs(arrival_times[index] - departure_times[index - 1]) + abs(arrival_times[index + 1] - departure_times[index])
    
    # Time saved is the difference
    return travel_time_with_place - travel_time_without_place

def optimize_with_time_constraint(route, arrival_times, departure_times, wait_times, df, start_time, end_time, api_key):
    """
    ...
    Parameters:
    - route: List of place names in the current route
    - arrival_times: List of arrival times for each place
            time_saved = calculate_time_saved_by_removing(route, i, departure_times, arrival_times) if 'calculate_time_saved_by_removing' in globals() else 0
    - wait_times: List of wait times for each place
    - df: DataFrame containing place information
    - start_time: Required start time in minutes from midnight
    - end_time: Required end time in minutes from midnight
    
    Returns:
    - Dictionary containing optimized route information or None if optimization fails
    """
    try:
        ...
        route_list, total_wait_time, itinerary, new_arrival_times, new_departure_times, new_wait_times, early_finish = calculate_detailed_itinerary(
            new_route, df, start_time, end_time, cache, api_key
        )
